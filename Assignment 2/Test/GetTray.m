classdef GetTray < handle
    ...
    methods
        % %% ...structors
        % function self = GetTray(cowCount)
        %     if 0 < nargin
        ...
        %         self.trayModel{i} = self.GettrayModel(['Service Tray',num2str(i)]);
        %         % Random spawn
        %         basePose = SE3(SE2((2 * rand()-1) * self.paddockSize(1)/2 ...
        %                                  , (2 * rand()-1) * self.paddockSize(2)/2 ...
        %                                  , (2 * rand()-1) * 2 * pi));
        %         self.trayModel{i}.base = basePose;
        % 
        %          % Plot 3D model
        %         plot3d(self.trayModel{i},0,'workspace',self.workspaceDimensions,'view',[-30,30],'delay',0,'noarrow','nowrist');
        ...
        %         handles = findobj('Tag', self.trayModel{index}.name);
        ...
        function PlotSingleRandomStep(self)
            for cowIndex = 1:self.cowCount
                % Move Forward
                self.trayModel{cowIndex}.base = self.trayModel{cowIndex}.base * SE3(SE2(0.2, 0, 0));
                animate(self.trayModel{cowIndex},0);
                
                % Turn randomly
                % Save base as a temp variable
                tempBase = self.trayModel{cowIndex}.base.T;
                ...
                % Update base pose
                self.trayModel{cowIndex}.base = newBase;
                animate(self.trayModel{cowIndex},0);                
                ...
                || self.workspaceDimensions(4) < tempBase(2,4)
                    self.trayModel{cowIndex}.base = self.trayModel{cowIndex}.base * SE3(SE2(-0.2, 0, 0)) * SE3(SE2(0, 0, pi));
                end
            end
