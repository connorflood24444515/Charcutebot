function SafetyFence(transformation_matrix4)
    if nargin < 1
    transformation_matrix4 = eye(4); % Default to identity matrix if no argument provided
    end

    % Load the mesh
    mesh_h = PlaceObject('ColouredSafetyFence.PLY');
    vertices = get(mesh_h, 'Vertices');

    % Scale the vertices
    scale = 0.0001;
    vertices = vertices * scale;

    % Create a combined transformation matrix
    combined_transformation = trotx(pi/2) * troty(pi/2) * transformation_matrix4;

    % Transform the vertices
    transformedVertices = [vertices, ones(size(vertices, 1), 1)] * combined_transformation';
    set(mesh_h, 'Vertices', transformedVertices(:, 1:3));
end



    % mesh_h = PlaceObject('ColouredSafetyFence.PLY'); 
    % vertices = get(mesh_h, 'Vertices');
    % 
    % transformedVertices = [vertices,ones(size(vertices,1),1)]';
    % set(mesh_h,'Vertices',transformedVertices(:,1:3));
    % 
    % scale = 0.0001;  %since object was too large upon import, scale of extinguisher was reduced.
    % vertices = vertices * scale;
    % 
    % transformedVertices = [vertices,ones(size(vertices,1),1)] * trotx(pi/2) * transl(1,1,1)';
    % set(mesh_h,'Vertices',transformedVertices(:,1:3));

 











%{
    mesh_h = PlaceObject('SafetyFence.PLY');
    vertices = get(mesh_h, 'Vertices');

    transformedVertices = [vertices,ones(size(vertices,1),1)]';
    set(mesh_h,'Vertices',transformedVertices(:,1:3));

    FenceColour = [0.99, 0.99, 0];  % RGB to produce yellow fence
    set(mesh_h, 'FaceColor', FenceColour); 
%}


